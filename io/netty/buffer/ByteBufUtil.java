...
public final class ByteBufUtil {
  ...
  static byte[] threadLocalTempArray(int minLength) {
    return (minLength <= 1024) ? (byte[])BYTE_ARRAYS.get() : PlatformDependent.allocateUninitializedArray(minLength);
  }
  ...
  public static int indexOf(ByteBuf needle, ByteBuf haystack) {
    ...
    int ell = Math.max((int)(suffixes >> 32L), (int)(prefixes >> 32L));
  ...
  public static byte[] getBytes(ByteBuf buf, int start, int length, boolean copy) {
    ...
    if (buf.hasArray()) {
      ...
      byte[] arrayOfByte = buf.array();
      if (copy || baseOffset != 0 || length != arrayOfByte.length)
        return Arrays.copyOfRange(arrayOfByte, baseOffset, baseOffset + length); 
      return arrayOfByte;
    } 
  ...
  static final class ThreadLocalUnsafeDirectByteBuf extends UnpooledUnsafeDirectByteBuf {
    ...
    private ThreadLocalUnsafeDirectByteBuf(ObjectPool.Handle<ThreadLocalUnsafeDirectByteBuf> handle) {
      ...
      this.handle = (Recycler.EnhancedHandle<ThreadLocalUnsafeDirectByteBuf>)handle;
    }
  ...
  static final class ThreadLocalDirectByteBuf extends UnpooledDirectByteBuf {
    ...
    private ThreadLocalDirectByteBuf(ObjectPool.Handle<ThreadLocalDirectByteBuf> handle) {
      super(UnpooledByteBufAllocator.DEFAULT, 256, 2147483647);
      this.handle = (Recycler.EnhancedHandle<ThreadLocalDirectByteBuf>)handle;
    }
