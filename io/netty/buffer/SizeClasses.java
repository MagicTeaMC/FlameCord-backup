...
abstract class SizeClasses implements SizeClassesMetric {
  ...
  protected SizeClasses(int pageSize, int pageShifts, int chunkSize, int directMemoryCacheAlignment) {
    int group = PoolArenasCache.log2(chunkSize) - 4 - 2 + 1;
    ...
    int ndeltaLimit = 4;
    int nDelta;
    for (nDelta = 0; nDelta < ndeltaLimit; nDelta++, nSizes++) {
    ...
    for (; size < chunkSize; log2Group++, log2Delta++) {
      for (nDelta = 1; nDelta <= ndeltaLimit && size < chunkSize; nDelta++, nSizes++) {
        short[] sizeClass = newSizeClass(nSizes, log2Group, log2Delta, nDelta, pageShifts);
        sizeClasses[nSizes] = sizeClass;
  ...
  private static short[] newSizeClass(int index, int log2Group, int log2Delta, int nDelta, int pageShifts) {
    ...
    int log2Ndelta = (nDelta == 0) ? 0 : PoolArenasCache.log2(nDelta);
  ...
  public int size2SizeIdx(int size) {
    ...
    int x = PoolArenasCache.log2((size << 1) - 1);
    int shift = (x < 7) ? 0 : (x - 6);
    int group = shift << 2;
  ...
  private int pages2pageIdxCompute(int pages, boolean floor) {
    ...
    if (pageSize > this.chunkSize)
      return this.nPSizes; 
    int x = PoolArenasCache.log2((pageSize << 1) - 1);
