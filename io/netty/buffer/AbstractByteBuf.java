...
public abstract class AbstractByteBuf extends ByteBuf {
  ...
  final void ensureWritable0(int minWritableBytes) {
    ...
    if (checkBounds && (targetCapacity < 0 || targetCapacity > this.maxCapacity)) {
      ensureAccessible();
      throw new IndexOutOfBoundsException(String.format("writerIndex(%d) + minWritableBytes(%d) exceeds maxCapacity(%d): %s", new Object[] { Integer.valueOf(writerIndex), Integer.valueOf(minWritableBytes), Integer.valueOf(this.maxCapacity), this }));
    } 
  ...
  public CharSequence getCharSequence(int index, int length, Charset charset) {
    if (CharsetUtil.US_ASCII.equals(charset) || CharsetUtil.ISO_8859_1.equals(charset))
      return (CharSequence)new AsciiString(ByteBufUtil.getBytes(this, index, length, true), false); 
  ...
  protected final void checkNewCapacity(int newCapacity) {
    ...
    if (checkBounds && (newCapacity < 0 || newCapacity > maxCapacity()))
      throw new IllegalArgumentException("newCapacity: " + newCapacity + " (expected: 0-" + 
          maxCapacity() + ')'); 
  }
