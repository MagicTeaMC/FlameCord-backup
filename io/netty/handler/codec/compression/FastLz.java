  ...
  static int compress(ByteBuf input, int inOffset, int inLength, ByteBuf output, int outOffset, int proposedLevel) {
    ...
    while (ip < ipLimit) {
      ...
      if (!matchLabel) {
        ...
        distance = (anchor - ref);
        htab[hslot] = anchor;
        if (distance == 0L || ((level == 1) ? (distance >= 8191L) : (distance >= 73725L)) || input
          
          .getByte(inOffset + ref++) != input.getByte(inOffset + ip++) || input
          .getByte(inOffset + ref++) != input.getByte(inOffset + ip++) || input
          .getByte(inOffset + ref++) != input.getByte(inOffset + ip++)) {
        ...
        if (level == 2)
          if (distance >= 8191L) {
            if (input.getByte(inOffset + ip++) != input.getByte(inOffset + ref++) || input
              .getByte(inOffset + ip++) != input.getByte(inOffset + ref++)) {
              output.setByte(outOffset + op++, input.getByte(inOffset + anchor++));
              ip = anchor;
              copy++;
              if (copy == 32) {
      ...
      if (level == 2) {
        if (distance < 8191L) {
          if (len < 7) {
            output.setByte(outOffset + op++, (byte)(int)((len << 5) + (distance >>> 8L)));
            output.setByte(outOffset + op++, (byte)(int)(distance & 0xFFL));
          } else {
            output.setByte(outOffset + op++, (byte)(int)(224L + (distance >>> 8L)));
          ...
          output.setByte(outOffset + op++, -1);
          output.setByte(outOffset + op++, (byte)(int)(distance >>> 8L));
          output.setByte(outOffset + op++, (byte)(int)(distance & 0xFFL));
        } else {
          distance -= 8191L;
          output.setByte(outOffset + op++, -1);
