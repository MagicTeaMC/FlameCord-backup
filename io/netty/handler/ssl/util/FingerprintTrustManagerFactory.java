...
public final class FingerprintTrustManagerFactory extends SimpleTrustManagerFactory {
    ...
    this.tlmd = new FastThreadLocal<MessageDigest>() {
        protected MessageDigest initialValue() {
          try {
            ...
            throw new IllegalArgumentException(
                String.format("Unsupported hash algorithm: %s", new Object[] { this.val$algorithm }), e);
          } 
        }
      };
    this.fingerprints = list.<byte[]>toArray(new byte[0][]);
  }
  ...
  static byte[][] toFingerprintArray(Iterable<String> fingerprints) {
    ...
    return list.<byte[]>toArray(new byte[0][]);
  }
