...
final class PlatformDependent0 {
  ...
  static {
    ...
    if (unsafe == null) {
      ...
      try {
        Object maybeDirectBufferConstructor = AccessController.doPrivileged(new PrivilegedAction() {
              public Object run() {
                try {
                  Constructor<?> constructor = (PlatformDependent0.javaVersion() >= 21) ? direct.getClass().getDeclaredConstructor(new Class[] { long.class, long.class }) : direct.getClass().getDeclaredConstructor(new Class[] { long.class, int.class });
    ...
    if (javaVersion() > 9) {
      ALIGN_SLICE = AccessController.<Method>doPrivileged(new PrivilegedAction() {
            public Object run() {
              try {
                return ByteBuffer.class.getDeclaredMethod("alignedSlice", new Class[] { int.class });
              } catch (Exception e) {
                return null;
              } 
            }
  ...
  static boolean equals(byte[] bytes1, int startPos1, byte[] bytes2, int startPos2, int length) {
    ...
    if (length >= 8) {
      ...
      long i;
      for (i = baseOffset1 - 8L + length; i >= end; i -= 8L) {
    ...
    if (remainingBytes >= 2)
      return (UNSAFE.getChar(bytes1, baseOffset1) == UNSAFE.getChar(bytes2, baseOffset2) && (remainingBytes == 2 || UNSAFE
        
        .getByte(bytes1, baseOffset1 + 2L) == UNSAFE.getByte(bytes2, baseOffset2 + 2L))); 
    return (remainingBytes == 0 || UNSAFE
      .getByte(bytes1, baseOffset1) == UNSAFE.getByte(bytes2, baseOffset2));
  }
