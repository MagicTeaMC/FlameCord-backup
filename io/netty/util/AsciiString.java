...
public final class AsciiString implements CharSequence, Comparable<CharSequence> {
  ...
  public AsciiString concat(CharSequence string) {
    ...
    if (string instanceof AsciiString) {
      ...
      if (isEmpty())
        return that; 
      byte[] arrayOfByte = PlatformDependent.allocateUninitializedArray(thisLen + thatLen);
  ...
  public boolean contentEqualsIgnoreCase(CharSequence string) {
    ...
    if (string instanceof AsciiString) {
      ...
      for (int k = arrayOffset(), m = rhs.arrayOffset(), n = k + length(); k < n; k++, m++) {
  ...
  public AsciiString[] split(char delim) {
    ...
    return res.<AsciiString>toArray(new AsciiString[0]);
  }
