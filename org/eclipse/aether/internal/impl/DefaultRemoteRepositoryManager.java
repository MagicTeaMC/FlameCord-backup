...
public class DefaultRemoteRepositoryManager implements RemoteRepositoryManager, Service {
  ...
  @Inject
  DefaultRemoteRepositoryManager(UpdatePolicyAnalyzer updatePolicyAnalyzer, ChecksumPolicyProvider checksumPolicyProvider) {
  ...
  public List<RemoteRepository> aggregateRepositories(RepositorySystemSession session, List<RemoteRepository> dominantRepositories, List<RemoteRepository> recessiveRepositories, boolean recessiveIsRaw) {
    ...
    for (RemoteRepository recessiveRepository : recessiveRepositories) {
      ...
      ListIterator<RemoteRepository> it;
  ...
  private void logMirror(RepositorySystemSession session, RemoteRepository original, RemoteRepository mirror) {
    ...
    LOGGER.debug("Using mirror {} ({}) for {} ({}).", new Object[] { mirror
          .getId(), mirror.getUrl(), original.getId(), original.getUrl() });
  }
  ...
  private RemoteRepository mergeMirrors(RepositorySystemSession session, RemoteRepository dominant, RemoteRepository recessive) {
    ...
    label18: for (RemoteRepository rec : recessive.getMirroredRepositories()) {
  ...
  private RepositoryPolicy merge(RepositorySystemSession session, RepositoryPolicy policy1, RepositoryPolicy policy2, boolean globalPolicy) {
    ...
    if (policy2 == null) {
      ...
      if (!globalPolicy || StringUtils.isEmpty(checksums))
        checksums = this.checksumPolicyProvider.getEffectiveChecksumPolicy(session, policy1.getChecksumPolicy(), policy2
            .getChecksumPolicy()); 
      String updates = session.getUpdatePolicy();
      if (!globalPolicy || StringUtils.isEmpty(updates))
        updates = this.updatePolicyAnalyzer.getEffectiveUpdatePolicy(session, policy1.getUpdatePolicy(), policy2
            .getUpdatePolicy()); 
