...
public final class Soft {
  ...
  public static <K, V> ConcurrentMap<K, V> concurrentKeys(int capacity, int concurrency) {
    return new MildConcurrentKeys<K, V>(new ConcurrentHashMap<Reference<K>, V>(capacity, 0.75F, concurrency), true);
  }
  ...
  public static <K, V> ConcurrentMap<K, V> concurrentValues(int capacity, int concurrency) {
    return new MildConcurrentValues<K, V>(new ConcurrentHashMap<K, Reference<V>>(capacity, 0.75F, concurrency), true);
  }
}
