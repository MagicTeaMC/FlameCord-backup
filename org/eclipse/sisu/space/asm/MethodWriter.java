...
final class MethodWriter extends MethodVisitor {
    ...
    if (this.compute == 0) {
        ...
        while (l != null) {
          if ((l.status & 0x80) != 0) {
            ...
            if ((subroutine.status & 0x400) == 0) {
              id++;
              subroutine.visitSubroutine(null, id / 32L << 32L | 
      ...
      while (stack != null) {
        ...
        while (b != null) {
          l = b.successor;
          if ((l.status & 0x8) == 0) {
            l.inputStackTop = (b.info == Integer.MAX_VALUE) ? 1 : (start + 
              b.info);
            l.status |= 0x8;
            l.next = stack;
            stack = l;
  ...
  private void resizeInstructions() {
    ...
    int j = 0;
    while (j < this.code.length) {
      int v, label, k, newOffset, opcode = b[j] & 0xFF;
      switch (ClassWriter.TYPE[opcode]) {
        case 0:
        case 4:
        ...
        case 9:
          if (opcode > 201) {
            opcode = (opcode < 218) ? (opcode - 49) : (opcode - 20);
            label = j + readUnsignedShort(b, j + 1);
          } else {
            label = j + readShort(b, j + 1);
          } 
          newOffset = getNewOffset(allIndexes, allSizes, j, label);
          if (resize[j]) {
            if (opcode == 167) {
              newCode.putByte(200);
            } else if (opcode == 168) {
        ...
        case 17:
          opcode = b[j + 1] & 0xFF;
          if (opcode == 132) {
            newCode.putByteArray(b, j, 6);
            j += 6;
            continue;
          } 
          newCode.putByteArray(b, j, 4);
          j += 4;
          continue;
        case 1:
        case 3:
        case 11:
          newCode.putByteArray(b, j, 2);
          j += 2;
          continue;
        case 2:
        case 5:
        ...
        case 12:
        case 13:
          newCode.putByteArray(b, j, 3);
          j += 3;
          continue;
        case 7:
        case 8:
          newCode.putByteArray(b, j, 5);
          j += 5;
          continue;
      } 
      newCode.putByteArray(b, j, 4);
    ...
    if (this.compute == 0) {
      ...
      for (int k = 0; k < this.cw.typeTable.length; k++) {
        Item item = this.cw.typeTable[k];
    ...
    for (i = 0; i < 2; i++) {
      ...
      if (bv != null) {
        ...
        j = 0;
        while (j < bv.length) {
          int label = readUnsignedShort(b, j);
          int newOffset = getNewOffset(allIndexes, allSizes, 0, label);
          writeShort(b, j, newOffset);
          label += readUnsignedShort(b, j + 2);
          newOffset = getNewOffset(allIndexes, allSizes, 0, label) - 
            newOffset;
          writeShort(b, j + 2, newOffset);
          j += 10;
        } 
      } 
    } 
    if (this.lineNumber != null) {
      ...
      j = 0;
      while (j < this.lineNumber.length) {
        writeShort(
            b, 
            j, 
            getNewOffset(allIndexes, allSizes, 0, 
              readUnsignedShort(b, j)));
        j += 4;
      } 
    } 
