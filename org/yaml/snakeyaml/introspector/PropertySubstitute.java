...
public class PropertySubstitute extends Property {
  ...
  public Object get(Object object) {
    try {
      ...
      throw new YAMLException("Unable to find getter for property '" + 
          getName() + "' on object " + object + ":" + e);
    } 
    if (this.delegate != null)
      return this.delegate.get(object); 
    throw new YAMLException("No getter or delegate for property '" + 
        getName() + "' on object " + object);
  }
  ...
  public void setTargetType(Class<?> targetType) {
    if (this.targetType != targetType) {
      ...
      if (this.field == null && log.isLoggable(Logger.Level.WARNING))
        log.warn(String.format("Failed to find field for %s.%s", new Object[] { targetType.getName(), getName() })); 
  ...
  private Method discoverMethod(Class<?> type, String name, Class<?>... params) {
    ...
    if (log.isLoggable(Logger.Level.WARNING))
      log.warn(String.format("Failed to find [%s(%d args)] for %s.%s", new Object[] { name, Integer.valueOf(params.length), this.targetType
              .getName(), getName() })); 
    return null;
  }
