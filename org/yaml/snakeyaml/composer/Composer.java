...
public class Composer {
    ...
    if (this.parser.checkEvent(Event.ID.Alias)) {
      ...
      if (!(node instanceof ScalarNode)) {
        ...
        if (this.nonScalarAliasesCount > this.loadingConfig.getMaxAliasesForCollections())
          throw new YAMLException("Number of aliases for non-scalar nodes exceeds the specified max=" + this.loadingConfig
  ...
  protected Node composeScalarNode(String anchor, List<CommentLine> blockComments) {
    ...
    if (tag == null || tag.equals("!")) {
      ...
      if (nodeTag.isCustomGlobal() && 
        !this.loadingConfig.getTagInspector().isGlobalTagAllowed(nodeTag))
        throw new ComposerException(null, null, "Global tag is not allowed: " + tag, ev
            .getStartMark()); 
    } 
  ...
  protected Node composeSequenceNode(String anchor) {
    ...
    if (tag == null || tag.equals("!")) {
      ...
      if (nodeTag.isCustomGlobal() && 
        !this.loadingConfig.getTagInspector().isGlobalTagAllowed(nodeTag))
        throw new ComposerException(null, null, "Global tag is not allowed: " + tag, startEvent
            .getStartMark()); 
    } 
