...
public class EnumUtils {
  ...
  public static <E extends Enum<E>> E getEnumIgnoreCase(Class<E> enumClass, String enumName) {
    if (enumName == null || !enumClass.isEnum())
      return null; 
    for (Enum enum_ : (Enum[])enumClass.getEnumConstants()) {
      if (enum_.name().equalsIgnoreCase(enumName))
        return (E)enum_; 
    } 
    return null;
  }
  ...
  public static <E extends Enum<E>> long[] generateBitVectors(Class<E> enumClass, Iterable<? extends E> values) {
    ...
    for (Enum enum_ : condensed)
      result[enum_.ordinal() / 64] = result[enum_.ordinal() / 64] | 1L << enum_.ordinal() % 64; 
  ...
  public static <E extends Enum<E>> EnumSet<E> processBitVectors(Class<E> enumClass, long... values) {
    ...
    for (Enum enum_ : (Enum[])enumClass.getEnumConstants()) {
      ...
      if (block < lvalues.length && (lvalues[block] & 1L << enum_.ordinal() % 64) != 0L)
        results.add((E)enum_); 
    } 
    return results;
  }
