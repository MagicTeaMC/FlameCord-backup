...
public class LocaleUtils {
  ...
  private static Locale parseLocale(String str) {
    ...
    if (segments.length == 2) {
      ...
      if (isISO639LanguageCode(language) && (country
        .isEmpty() || isISO3166CountryCode(country) || isNumericAreaCode(country)) && 
        !variant.isEmpty())
  ...
  public static List<Locale> languagesByCountry(String countryCode) {
    ...
    if (langs == null) {
      ...
      for (Locale locale : locales) {
        if (countryCode.equals(locale.getCountry()) && locale
          .getVariant().isEmpty())
          langs.add(locale); 
      } 
    ...
    if (countries == null) {
      ...
      for (Locale locale : locales) {
        if (languageCode.equals(locale.getLanguage()) && 
          !locale.getCountry().isEmpty() && locale
          .getVariant().isEmpty())
