...
public class TypeUtils {
  ...
  private static final class ParameterizedTypeImpl implements ParameterizedType {
    ...
    private ParameterizedTypeImpl(Class<?> raw, Type useOwner, Type[] typeArguments) {
      ...
      this.typeArguments = Arrays.<Type, Type>copyOf(typeArguments, typeArguments.length, Type[].class);
    }
  ...
  private static boolean isAssignable(Type type, Class<?> toClass) {
    ...
    if (type instanceof GenericArrayType)
      return (toClass.equals(Object.class) || (toClass
        .isArray() && 
  ...
  private static Map<TypeVariable<?>, Type> getTypeArguments(ParameterizedType parameterizedType, Class<?> toClass, Map<TypeVariable<?>, Type> subtypeVarAssigns) {
    ...
    for (int i = 0; i < arrayOfTypeVariable.length; i++) {
      ...
      typeVarAssigns.put(arrayOfTypeVariable[i], typeVarAssigns.containsKey(typeArg) ? typeVarAssigns
          .get(typeArg) : typeArg);
    } 
  ...
  private static <T> void mapTypeVariablesToArguments(Class<T> cls, ParameterizedType parameterizedType, Map<TypeVariable<?>, Type> typeVarAssigns) {
    ...
    for (int i = 0; i < typeArgs.length; i++) {
      ...
      Type typeArg = typeArgs[i];
      if (typeVarList.contains(typeArg) && typeVarAssigns
  ...
  public static boolean isInstance(Object value, Type type) {
    ...
    return (value == null) ? ((!(type instanceof Class) || !((Class)type).isPrimitive())) : 
      isAssignable(value.getClass(), type, (Map<TypeVariable<?>, Type>)null);
  }
  ...
  public static final ParameterizedType parameterizeWithOwner(Type owner, Class<?> raw, Type... typeArguments) {
    ...
    Validate.isTrue(((raw.getTypeParameters()).length == typeArguments.length), "invalid number of type parameters specified: expected %d, got %d", new Object[] { Integer.valueOf((raw.getTypeParameters()).length), 
  ...
  private static String classToString(Class<?> c) {
    ...
    if ((c.getTypeParameters()).length > 0) {
      buf.append('<');
      appendAllTo(buf, ", ", (Object[])c.getTypeParameters());
      buf.append('>');
    } 
    return buf.toString();
  }
  ...
  private static void appendRecursiveTypes(StringBuilder buf, int[] recursiveTypeIndexes, Type[] argumentTypes) {
    for (int i = 0; i < recursiveTypeIndexes.length; i++) {
      appendAllTo(buf.append('<'), ", ", new String[] { argumentTypes[i].toString() }).append('>');
    } 
  ...
  private static int[] findRecursiveTypes(ParameterizedType p) {
    Type[] filteredArgumentTypes = Arrays.<Type>copyOf(p.getActualTypeArguments(), (p.getActualTypeArguments()).length);
    int[] indexesToRemove = new int[0];
    for (int i = 0; i < filteredArgumentTypes.length; i++) {
      if (filteredArgumentTypes[i] instanceof TypeVariable && 
        containsVariableTypeSameParametrizedTypeBound((TypeVariable)filteredArgumentTypes[i], p))
  ...
  private static boolean containsVariableTypeSameParametrizedTypeBound(TypeVariable<?> typeVariable, ParameterizedType p) {
    return ArrayUtils.contains((Object[])typeVariable.getBounds(), p);
  }
