...
public abstract class AbstractLogger implements ExtendedLogger, LocationAwareLogger, Serializable {
  ...
  protected EntryMessage enter(String fqcn, String format, Supplier<?>... paramSuppliers) {
    ...
    if (isEnabled(Level.TRACE, ENTRY_MARKER, (Object)null, (Throwable)null))
      logMessageSafely(fqcn, Level.TRACE, ENTRY_MARKER, 
          (Message)(entryMsg = this.flowMessageFactory.newEntryMessage(format, LambdaUtil.getAll((Supplier[])paramSuppliers))), (Throwable)null); 
    return entryMsg;
  }
  ...
  protected EntryMessage enter(String fqcn, String format, Object... params) {
    EntryMessage entryMsg = null;
    if (isEnabled(Level.TRACE, ENTRY_MARKER, (Object)null, (Throwable)null))
      logMessageSafely(fqcn, Level.TRACE, ENTRY_MARKER, (Message)(entryMsg = this.flowMessageFactory.newEntryMessage(format, params)), (Throwable)null); 
    return entryMsg;
  }
  ...
  protected EntryMessage entryMsg(String format, Supplier<?>... paramSuppliers) {
    return entryMsg(format, LambdaUtil.getAll((Supplier[])paramSuppliers));
  }
  ...
  protected <R> R exit(String fqcn, R result) {
    if (isEnabled(Level.TRACE, EXIT_MARKER, (CharSequence)null, (Throwable)null))
      logMessageSafely(fqcn, Level.TRACE, EXIT_MARKER, (Message)this.flowMessageFactory.newExitMessage(null, result), (Throwable)null); 
    return result;
  }
  
  protected <R> R exit(String fqcn, String format, R result) {
    if (isEnabled(Level.TRACE, EXIT_MARKER, (CharSequence)null, (Throwable)null))
      logMessageSafely(fqcn, Level.TRACE, EXIT_MARKER, (Message)this.flowMessageFactory.newExitMessage(format, result), (Throwable)null); 
    return result;
  }
  
  protected Message exitMsg(String format, Object result) {
    return (Message)this.flowMessageFactory.newExitMessage(format, result);
  }
  ...
  protected LogBuilder getLogBuilder(Level level) {
    ...
    return (Constants.ENABLE_THREADLOCALS && !builder.isInUse()) ? builder.reset(this, level) : (LogBuilder)new DefaultLogBuilder(this, level);
  }
}
