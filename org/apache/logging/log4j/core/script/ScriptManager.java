...
public class ScriptManager implements FileWatcher {
  ...
  public ScriptManager(Configuration configuration, WatchManager watchManager, String scriptLanguages) {
    ...
    this
      .allowedLanguages = (Set<String>)Arrays.<String>stream(Strings.splitList(scriptLanguages)).map(String::toLowerCase).collect(Collectors.toSet());
  ...
  public boolean addScript(AbstractScript script) {
    if (this.allowedLanguages.contains(script.getLanguage().toLowerCase(Locale.ROOT))) {
      ...
      logger.error("Unable to add script {}, {} has not been configured as an allowed language", script
          .getName(), script.getLanguage());
      return false;
    } 
    return true;
  }
