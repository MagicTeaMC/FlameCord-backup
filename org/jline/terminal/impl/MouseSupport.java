...
public class MouseSupport {
  ...
  public static MouseEvent readMouse(IntSupplier reader, MouseEvent last) {
    ...
    if ((cb & 0x40) == 64) {
      ...
      button = ((cb & 0x1) == 1) ? MouseEvent.Button.WheelDown : MouseEvent.Button.WheelUp;
    } else {
      int b = cb & 0x3;
      switch (b) {
        case 0:
          button = MouseEvent.Button.Button1;
          if (last.getButton() == button && (last
            .getType() == MouseEvent.Type.Pressed || last.getType() == MouseEvent.Type.Dragged)) {
