...
public class UpdatableResultSet extends ResultSetImpl {
    ...
    try {
      synchronized (checkClosed().getConnectionMutex()) {
        ...
        for (int i = 0; i < numKeys; i++) {
          int index = ((Integer)this.primaryKeyIndices.get(i)).intValue();
  ...
  private void setParamValue(ClientPreparedStatement ps, int psIdx, Row row, int rsIdx, Field field) throws SQLException {
    ...
    switch (field.getMysqlType()) {
      ...
      case DATE:
        ...
        return;
      case TIMESTAMP:
        ps.setObject(psIdx, getObject(rsIdx + 1, Timestamp.class), (SQLType)MysqlType.TIMESTAMP, field.getDecimals());
  ...
  protected void syncUpdate() throws SQLException {
    ...
    for (int j = 0; j < numKeys; j++) {
      int idx = ((Integer)this.primaryKeyIndices.get(j)).intValue();
