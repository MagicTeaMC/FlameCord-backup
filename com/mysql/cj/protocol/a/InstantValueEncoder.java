    ...
    switch (binding.getMysqlType()) {
      ...
      case TIMESTAMP:
        x = adjustTimestamp(
            Timestamp.valueOf(((Instant)binding.getValue()).atOffset(ZoneOffset.UTC)
              .atZoneSameInstant(this.serverSession.getDefaultTimeZone().toZoneId()).toLocalDateTime()), binding
      ...
      case LONGTEXT:
        ...
        sb.append(((Instant)binding.getValue()).atOffset(ZoneOffset.UTC)
            .format((((Boolean)this.sendFractionalSeconds.getValue()).booleanValue() && ((Instant)binding.getValue()).getNano() > 0) ? TimeUtil.DATETIME_FORMATTER_WITH_NANOS_WITH_OFFSET : TimeUtil.DATETIME_FORMATTER_NO_FRACT_WITH_OFFSET));
        sb.append("'");
        return sb.toString();
    } 
  ...
  public void encodeAsBinary(Message msg, BindValue binding) {
    ...
    switch (binding.getMysqlType()) {
      ...
      case TIMESTAMP:
        ...
        return;
      case YEAR:
        intoPacket.writeInteger(NativeConstants.IntegerDataType.INT4, ((Instant)binding.getValue()).atOffset(ZoneOffset.UTC)
            .atZoneSameInstant(this.serverSession.getDefaultTimeZone().toZoneId()).getYear());
        return;
      case CHAR:
      case VARCHAR:
      ...
      case LONGTEXT:
        intoPacket.writeBytes(NativeConstants.StringSelfDataType.STRING_LENENC, 
            StringUtils.getBytes(((Instant)binding.getValue()).atOffset(ZoneOffset.UTC)
              .format((((Boolean)this.sendFractionalSeconds.getValue()).booleanValue() && ((Instant)binding.getValue()).getNano() > 0) ? TimeUtil.DATETIME_FORMATTER_WITH_NANOS_WITH_OFFSET : TimeUtil.DATETIME_FORMATTER_NO_FRACT_WITH_OFFSET), (String)this.charEncoding
