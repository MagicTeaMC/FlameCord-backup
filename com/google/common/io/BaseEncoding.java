...
public abstract class BaseEncoding {
  ...
  private static final class Alphabet {
    ...
    int decode(char ch) throws BaseEncoding.DecodingException {
      if (ch > '') {
        ...
        throw new BaseEncoding.DecodingException((String.valueOf(Integer.toHexString(ch)).length() != 0) ? "Unrecognized character: 0x".concat(String.valueOf(Integer.toHexString(ch))) : new String("Unrecognized character: 0x"));
      } 
      ...
      return new InputStream() {
          ...
          public int read() throws IOException {
            while (true) {
              ...
              if (this.hitPadding) {
                int i = this.readChars;
                throw new BaseEncoding.DecodingException((new StringBuilder(61)).append("Expected padding character but found '").append(ch).append("' at index ").append(i).toString());
              } 
  ...
  static final class Base16Encoding extends StandardBaseEncoding {
    ...
    int decodeTo(byte[] target, CharSequence chars) throws BaseEncoding.DecodingException {
      ...
      if (chars.length() % 2 == 1) {
        int j = chars.length();
        throw new BaseEncoding.DecodingException((new StringBuilder(32)).append("Invalid input length ").append(j).toString());
      } 
      int bytesWritten = 0;
