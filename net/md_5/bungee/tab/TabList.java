  ...
  private static void rewrite(PlayerListItem.Item item) {
    ...
    if (player != null) {
      ...
      if (item.getGamemode() != null)
        player.setGamemode(item.getGamemode().intValue()); 
      if (item.getPing() != null)
        player.setPing(item.getPing().intValue()); 
    } 
  }
  
  public abstract void onUpdate(PlayerListItem paramPlayerListItem);
  
  public abstract void onUpdate(PlayerListItemRemove paramPlayerListItemRemove);
  
  public abstract void onUpdate(PlayerListItemUpdate paramPlayerListItemUpdate);
