...
public class CommandSend extends Command implements TabExecutor {
      ...
      for (Map.Entry<ServerConnectRequest.Result, List<String>> entry : this.results.entrySet()) {
        ...
        if (!((List)entry.getValue()).isEmpty())
          builder.event(new HoverEvent(HoverEvent.Action.SHOW_TEXT, (new ComponentBuilder(
                  Joiner.on(", ").join(entry.getValue()))).color(ChatColor.YELLOW).create())); 
        builder.append(((ServerConnectRequest.Result)entry.getKey()).name() + ": ").color(ChatColor.GREEN);
        builder.append("" + ((List)entry.getValue()).size()).bold(true);
        this.sender.sendMessage(builder.create());
      } 
    }
    
    public static class Entry implements Callback<ServerConnectRequest.Result> {
      private final CommandSend.SendCallback callback;
      
      private final ProxiedPlayer player;
      
      private final ServerInfo target;
      
      public Entry(CommandSend.SendCallback callback, ProxiedPlayer player, ServerInfo target) {
      ...
      public void done(ServerConnectRequest.Result result, Throwable error) {
        ((List<String>)this.callback.results.get(result)).add(this.player.getName());
        if (result == ServerConnectRequest.Result.SUCCESS)
          this.player.sendMessage(ProxyServer.getInstance().getTranslation("you_got_summoned", new Object[] { this.target.getName(), CommandSend.SendCallback.access$200(this.callback).getName() })); 
  ...
  public Iterable<String> onTabComplete(CommandSender sender, String[] args) {
    if (args.length > 2 || args.length == 0)
      return (Iterable<String>)ImmutableSet.of(); 
