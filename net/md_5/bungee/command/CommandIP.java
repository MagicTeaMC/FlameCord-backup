  ...
  public void execute(CommandSender sender, String[] args) {
    ...
    if (user == null) {
      ...
      sender.sendMessage((new ComponentBuilder())
          .appendLegacy(ProxyServer.getInstance().getTranslation("command_ip", new Object[] { user.getName(), user.getSocketAddress() })).event(new ClickEvent(ClickEvent.Action.COPY_TO_CLIPBOARD, user.getSocketAddress().toString()))
          .create());
    } 
  }
  
  public Iterable<String> onTabComplete(CommandSender sender, String[] args) {
    return (args.length == 1) ? super.onTabComplete(sender, args) : Collections.<String>emptyList();
  }
}
