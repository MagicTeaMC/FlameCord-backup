...
public class ConnectionThrottle {
  ...
  ConnectionThrottle(Ticker ticker, int throttleTime, int throttleLimit) {
    this
      ...
      .throttle = CacheBuilder.newBuilder().ticker(ticker).concurrencyLevel(Runtime.getRuntime().availableProcessors()).initialCapacity(100).expireAfterWrite(throttleTime, TimeUnit.MILLISECONDS).build(new CacheLoader<InetAddress, AtomicInteger>() {
  ...
  public boolean throttle(SocketAddress socketAddress) {
    if (!(socketAddress instanceof InetSocketAddress))
      return false; 
    InetAddress address = ((InetSocketAddress)socketAddress).getAddress();
    int throttleCount = ((AtomicInteger)this.throttle.getUnchecked(address)).incrementAndGet();
